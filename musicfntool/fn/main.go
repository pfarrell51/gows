...
import (
	"flag"
	"fmt"
	"github.com/pfarrell51/gows/musicfntool"
	"os"
	"path"
	"time"
)
...
func main() {
	...
	var helpflag bool
	var flags = new(musicfntool.FlagST)
	...
	flag.BoolVar(&flags.DoRename, "r", false, "rename - output rename from parsed file name")
	...
	musicfntool.SetFlagArgs(*flags)
	musicfntool.LoadArtistMap()

	if false {
		ch := make(chan musicfntool.Song)
		v := <-ch
		fmt.Println(v)
	}
	if flags.ZDumpArtist {
		musicfntool.DumpGptree()
		return
	}
...
func ProcessFiles(pathArg string) {
	if !musicfntool.GetFlags().ZDumpArtist {
		rmap := musicfntool.WalkFiles(pathArg)
		musicfntool.ProcessMap(pathArg, rmap)
	} else {
		musicfntool.DumpGptree()
	}
}
